...
- **uml_single_diagram_only.json** — One **UMLDiagram** plus **PreviewImage** (`IMAGE` link). Used for CI: ComfyUI requires output endpoints; comfy-test’s converter keeps nodes that have a connected output.
...
Comfy-test runs the full level pipeline (including **static_capture** and **validation** before **execution**; see [Testing.md](Testing.md) and the [comfy-test README](https://github.com/PozzettiAndrea/comfy-test#test-levels)) against the `cpu` list in `comfy-test.toml` and `pyproject.toml`. The default suite is **uml_single_diagram_only.json**, **uml_single_node.json**, **uml_mermaid.json**, and **uml_plantuml.json**. **uml_llm_ollama.json** is for manual loads; add it to **`cpu`** only if you want comfy-test to execute it. To add more workflows, add JSON under **workflows/** and append the filename to the `cpu` array in both config files. See [Testing.md](Testing.md) for mock LLM behavior when using LLM nodes in Actions.
