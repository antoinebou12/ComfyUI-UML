...
Comfy-test runs the full level pipeline (including **static_capture** and **validation** before **execution**; see [Testing.md](Testing.md) and the [comfy-test README](https://github.com/PozzettiAndrea/comfy-test#test-levels)) against the `cpu` list in `comfy-test.toml` and `pyproject.toml`. The suite runs all workflows: **uml_single_diagram_only.json**, **uml_single_node.json**, **uml_mermaid.json**, **uml_plantuml.json**, and **uml_llm_ollama.json**. The LLM workflow runs with a mocked LLM in CI (see `use_mock_llm()` in `nodes/uml_llm_shared.py` and [Testing.md](Testing.md)). To add more workflows, add JSON under **workflows/** and append the filename to the `cpu` array in both config files. See [Testing.md](Testing.md).
