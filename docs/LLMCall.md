# LLM Call (OpenAI/Anthropic/Gemini/Ollama)

Calls an LLM (OpenAI, Anthropic, Gemini, or Ollama) with a prompt and optional negative prompt. The **text** output is compatible with the UML Render node's **code_input** for generating diagram source from natural language.

## Parameters

- **prompt**: The main prompt text (multiline).
- **provider**: `openai`, `anthropic`, `gemini`, or `ollama`.
- **model**: Model name (e.g. gpt-4o-mini, claude-3-5-haiku-20241022, gemini-2.0-flash, llama3.2).
- **negative_prompt**: Optional; passed as a second message or instruction where supported.
- **api_key**: Optional; if empty, uses `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `GEMINI_API_KEY` from the environment as appropriate. Not used when provider is `ollama`.
...
- **OpenAI / Anthropic / Gemini**: Set `OPENAI_API_KEY`, `ANTHROPIC_API_KEY`, or `GEMINI_API_KEY` in the environment (or set **api_key** in the node). For Gemini, get an API key from [Google AI Studio](https://ai.google.dev/).
